...
class CGSetVisibility extends BaseNode {
  ...
  _registerResetCallback(sys, object, callbackFunctionArray, argsArray) {
    ...
    const key = object.guid.toString();
    if (!EffectReset.getInstance().propertyInitValueMap.has(key)) {
      ...
      EffectReset.getInstance().propertyInitValueMap.set(key, callBackFuncMap);
    }
  }

  _updateScriptComponentRecursively(entity, enable) {
    const components = entity.getComponents('JSScriptComponent');
    if (components && components.length > 0) {
      for (let i = 0; i < components.length; i++) {
        const comps = components[i];
        const key = comps.guid.toString();
        ...
        const compsEnable = EffectReset.getInstance().compsEnableMap.get(key);
        if (compsEnable === undefined) {
          EffectReset.getInstance().compsEnableMap.set(key, comps.enabled);
        }
        comps.enabled = enable && EffectReset.getInstance().compsEnableMap.get(key);
      }
    }
